...
automatically) and only proceeds if `now` is within
`SCHEDULE_TOLERANCE_MINUTES` of that target — see `time_manager.py` and
`jobs.is_job_due()`. This is what makes the DST-sensitive Gold/Dow
workflows safe to schedule with two static cron lines (one per DST
state) without ever double-sending.
...
> Asia/Tehran has used a fixed UTC+03:30 offset (no DST) since 2022, so
> Bitcoin/weekly workflows only need a single cron line each. Gold/Dow
> daily workflows carry **two** cron lines (EST and EDT UTC-equivalents)
> because NYSE open shifts with US daylight saving; `is_due()` rejects
> whichever line isn't DST-correct on a given day.

All times are computed against `America/New_York` and `Asia/Tehran` via
`zoneinfo` — no hardcoded UTC offsets anywhere in the business logic,
only in the workflow YAML (which is inherently static).
