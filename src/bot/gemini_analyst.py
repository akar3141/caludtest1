...
Write a 4-5 sentence professional summary in Persian (Farsi) covering price
action, which session had the highest volume, and the overall tone for the
day. Then add 1-2 sentences with a short near-term outlook based only on the
data above (trend continuation vs. reversal risk, key level to watch) —
do not invent data that is not given above. No headers, plain prose only.
...
class GeminiAnalyst:
    ...
    def summarize_daily(
        ...
        session_lines = "\n".join(
            f"- {s.name}: volume={s.volume}" for s in stats.sessions
        )
