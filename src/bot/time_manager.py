...
UTC_TZ = ZoneInfo("UTC")
# Broker/MT5 server time. Fixed UTC+3 (no DST) — this is the convention
# stated by the user for their broker and is independent of NY/Tehran DST
# handling above; it is used purely for display alongside Tehran time in
# reports, never for scheduling decisions.
...
class TimeManager:
    ...
    def to_server(self, dt: datetime) -> datetime:
        """Converts any aware datetime to the broker/server time (fixed UTC+3)."""
        if dt.tzinfo is None:
            raise ValueError("to_server() requires a timezone-aware datetime")
    ...
    def format_dual_time(self, dt: datetime, fmt: str = "%H:%M") -> str:
        """Formats a moment as both broker/server time and Iran time, since
        the two are offset by 30 minutes (server GMT+3 vs. Iran UTC+3:30)
        and reports should always show both to avoid ambiguity."""
