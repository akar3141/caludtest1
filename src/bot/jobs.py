...
def is_job_due(
    ...
    target_utc = target.target_time.astimezone(UTC_TZ)
    delay_minutes = (now - target_utc).total_seconds() / 60.0
    if delay_minutes > 0:
        logger.info(
            "JOB_IS_DUE reason=late_trigger delay_minutes=%.1f "
        ...
        logger.info(
            "JOB_IS_DUE reason=on_time_or_early "
            "asset=%s mode=%s target=%s now=%s",
            asset,
            mode,
            target.target_time.isoformat(),
